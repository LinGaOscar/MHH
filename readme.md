...
*   **分行經辦**: 發起電文下載申請後，狀態會更新為「待放行」。
*   **分行主管**: 在待辦事項中點選審核，確認無誤後點選「放行」。
*   **參數經辦**: 發起「自訂角色申請」或「自定義使用者表格 `USER_CUST`」維護申請後，狀態更新為「待放行」。
*   **參數主管**: 在待辦事項中審核 `USER_CUST` 或角色變更，確認無誤後點選「放行」。
...
*   **PDF Merge (ReservationMergeJob)**: 每 5 分鐘執行一次，合併已放行之預約電文。

### 2.4 日誌查詢 (SYS_LOGS, USER_LOGS, JOBS_LOGS)
*   **SYS_LOGS**: 用於排除系統錯誤、連線異常以及 AOP 攔截之系統例外。
*   **USER_LOGS**: 符合金管稽核需求，記錄所有使用者查詢、下載、申請與審核行為。
*   **JOBS_LOGS**: 用於監控所有背景任務 (Spring Batch) 的執行狀態、耗時及失敗錯誤堆疊。
