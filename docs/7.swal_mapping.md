# 7.4 SWAL 欄位對照 (Field Mapping)

本文件詳述外部 Oracle SWAL 資料庫與 MHH 的欄位映射關係，包含搜尋欄位、PARAMETERS JSON 與同步策略。
...
## 2. 進電搜尋欄位對照（MSG_INCOMING ← SWMIMSG）
...
## 3. 使用者需求擴充搜尋欄位 (已整併至 MSG_INCOMING)

以下欄位為原有 `user需求.md` 之規劃，已全數對應並整合至 MHH Schema 中。  
由於 SWAL `SWMIMSG` 未直接存放這些值，須於同步時從 `SWIMTX` CLOB（MT_CONTENT / MX_CONTENT）解析後寫入。
