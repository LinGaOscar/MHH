# 4.3 Batch 模組設計 (Batch Module Design)

本文件詳述 `mhh-batch` 模組的執行流程、任務分配與策略解析模式。
...
| 批次處理 | 自訂 `MhhJob` 介面 + `TaskScheduler`（非 Spring Batch） |
...
| Job | 說明 | 資料流向 |
|:--|:--|:--|
| `SwalSyncJob` | 從外部 Oracle SWAL 讀取電文（IMSG + OMSG 兩條路徑） | IMSG（`SWMIMSG`/`SWIMTX`）→ `MSG_INCOMING` / `MSG_INCOMING_TX`；OMSG（`SWMOMSG`/`SWOMTX`）→ `MSG_OUTGOING` / `MSG_OUTGOING_TX` |
...
| `JobSyncScheduler` | 動態同步 `JOBS_CONF`，更新執行中的排程（每 20 分鐘 + 啟動時立即執行） | — |
...
系統啟動
   │
   ├─→ CommandLineRunner 觸發 JobSyncScheduler.syncJobs()（立即執行）
   │       │
   ...
   │              └─ [無變動] 略過
   │
   └─→ @Scheduled(fixedRate) 每 20 分鐘重複上述流程
```
...
- [x] **步驟 1**：專案初始化與模組結構（依賴 `mhh-common`）
- [x] **步驟 2**：動態排程框架（`DynamicJobService`、`JobSyncScheduler`）
- [x] **步驟 3**：`SwalSyncJob` 實作（SWAL IMSG → `MSG_INCOMING` / `MSG_INCOMING_TX`；OMSG → `MSG_OUTGOING` / `MSG_OUTGOING_TX`）
- [x] **步驟 4**：`PdfImportJob` + 策略模式 Parser 實作（`Mt103Parser`、`MtGenericParser`、`MxGenericParser`）
- [x] **步驟 5**：`LogCleanupJob`（清 `USER_LOGS`）；本機日誌由 `logback-spring.xml` 管理
