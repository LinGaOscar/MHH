# 4.2 AP 模組設計 (AP Module Design)

本文件詳述 `mhh-ap` 模組的內部架構、安全性與稽核機制。
...
- [x] **步驟 1**：專案初始化（Maven 多模組、Thymeleaf 配置）
- [x] **步驟 2**：基礎架構開發（`UserActionAspect` AOP 日誌、`AuthEventListener` 登入稽核、Spring Security logout handler）
- [ ] **步驟 3**：Service 層核心邏輯（PDF 合併與電文查詢）
- [ ] **步驟 4**：Controller 接口逐一實作（`MsgQueryController`、`MsgDownloadController` 等）
- [ ] **步驟 5**：整合測試（Maker-Checker 流程 + SSO 流程驗證）
