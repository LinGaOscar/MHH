 ...
 * 進電查詢 API。
 *
 * 列表查詢（GET /api/incoming）：只存取 MSG_INCOMING 搜尋表，不載入電文大欄位。
 * 電文內容（GET /api/incoming/{messageId}/content）：存取 MSG_INCOMING_TX 電文表。
 */
@RestController
@RequestMapping("/api/incoming")
@RequiredArgsConstructor
public class MsgIncomingController {
    ...
    /** 進電列表查詢（分頁，僅搜尋表欄位） */
    @GetMapping
    @LogAction(action = "SEARCH_INCOMING", description = "查詢進電列表")
    public Map<String, Object> list(
            ...
            @RequestParam(required = false) String osnFrom,
            @RequestParam(required = false) String osnTo,
        ...
        Page<MsgIncoming> result = incomingRepository.findByFilters(
                from, to, typePrefix,
                blank(sender), blank(receiver),
                amtFrom, amtTo,
                blank(reference), blank(tag20), blank(tag21),
                blank(osnFrom), blank(osnTo),
                blank(unitCode), blank(amlFlag), blank(amlStatus), blank(flowStatus),
    ...
    private Map<String, Object> toRow(MsgIncoming m) {
